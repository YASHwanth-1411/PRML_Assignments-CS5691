...
def train_models():
    ...
    with open('models.pkl', 'wb') as file:
        pickle.dump(vectorizer_binary, file)
        pickle.dump(vectorizer_count, file)
        pickle.dump(vectorizer_tfidf, file)
        pickle.dump(weights, file)
        pickle.dump(bias, file)
        pickle.dump(svm, file)
        pickle.dump(w_lr, file)

def print_predictions():
    with open('models.pkl', 'rb') as f:
        vectorizer_binary = pickle.load(f)
        vectorizer_count = pickle.load(f)
        vectorizer_tfidf = pickle.load(f)
        weights_nb = pickle.load(f)
        bias_nb = pickle.load(f)
        svm = pickle.load(f)
        weights_lr = pickle.load(f)
    ...
    while True:
        ...
        with open(file_path, 'r') as file:
            ...
            bnb_prediction = 1 if np.dot(mail_binary_array, weights_nb) + bias_nb >= 0 else 0
            ...
            lr_prediction = 1 if np.dot(weights_lr, mail_tfidf_array[0]) >= 0 else 0
...
if not os.path.exists('models.pkl') or os.path.getmtime('emails.csv') > os.path.getmtime('models.pkl'): 
    train_models()
